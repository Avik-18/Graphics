//Triangle drawing using Bresenham line drawing algorithm and also interior colouring using Scan Line algorithm.
...
void ExportPixels()
{
	FILE *fp = fopen("1039_Pixels_2.txt", "w");
    for(int i=0; i<n; i++)
	{
		if(i>0 && i<exterior)
		{
 	...
 	printf("1039_Pixels_2.txt created successfully!\n");
}
void Export2SVG(int width, int height)
{
	...
	fprintf(fp,"\n<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" style=\"fill:red;stroke:black;stroke-width:.30;fill-opacity:0.76;stroke-opacity:0.90\"/>", scale*((x1+x2+x3)/3)-a1+100, scale*((y1+y2+y3)/3)-b1+100, scale, scale);  
  	fclose(fp);
  	printf("1039_2.svg created successfully!\n");
}
void Export2HTML(int width, int height)
{
    ...
    fprintf(fp, "\n</body>\n</html>\n");
    fclose(fp);
  	printf("1039_2.html created successfully!\n");
}
